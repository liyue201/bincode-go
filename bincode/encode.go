...
func Encode(data any) ([]byte, error) {
	return encodeData(reflect.ValueOf(data))
}

func MustEncode(data interface{}) []byte {
	serializedData, err := encodeData(reflect.ValueOf(data))
	if err != nil {
		panic(err)
	}
...
func encodeData(v reflect.Value) ([]byte, error) {
	switch v.Kind() {
	case reflect.Bool:
		if v.Bool() {
			return []byte{1}, nil
	...
	case reflect.Slice:
		b := make([]byte, 4)
		binary.LittleEndian.PutUint32(b, uint32(v.Len()))
